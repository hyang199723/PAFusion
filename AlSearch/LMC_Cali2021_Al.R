...
LMC_fit=function(Y1,Y2, X, s1,s2, 
                 rangeu=177,rangev=111,
                 initA=1,updateA=TRUE,
                 ...
                 match1=NULL,match2=NULL)
{
  ...
  Xlong <- NULL
  for(j in 1:p2){Xlong <- cbind(Xlong,as.vector(X[n1+1:n2,,j]))}
  betav <- lm(as.vector(Y2)~Xlong-1)$coef

  beta_a    <- rep(0,p_alpha)
  beta_a[1] <- initA
...
for(j in 1:n2){Z2[j,] <- fft_real(A*U2[j,]+V2[j,],inverse=TRUE)}
    ...
    # if(!plume_bias){Pv[4,4] <- Pv[5,5] <- Pv[6,6] <- 10^8}
    for (l in 1:nt) {
...
# Don't update betav anymore since the bias factors are in betau
#      Pv = Pv + t(X2_star[,l,])%*% SigmaV_inv%*% X2_star[,l,]/sig2[l]
#      Mv = Mv + t(X2_star[,l,])%*% SigmaV_inv%*% V2[, l]/sig2[l]
    }
    Pu_inv         <- solve(Pu)
    MMMu           <- Pu_inv %*% Mu
   ...
   #par(mfrow=c(3,2))
   #for(j in 3:6){
   #  plot(betau_all[j,1:i],type="l");abline(0,0,col=2)
   #} 
  # plot(log(sig1))
  # plot(log(sig2))
  }
  ...
  out=list(betau=betau_all, betav=betav_all, beta_a=beta_a_all,
